...
class SM2Suite(object):
    ...
    def encrypt(self, data):
        ...
        C1 = self.kg(int(k, 16), self.ecc_table['g'])
        xy = self.kg(int(k, 16), self.public_key)
        x2 = xy[0:self.para_len]
        y2 = xy[self.para_len:2 * self.para_len]
        ml = len(msg)
        t = sm3.sm3_kdf(xy.encode('utf8'), ml / 2)
        if int(t, 16) == 0:
            return self.encrypt(bytes.fromhex(data))
        else:
            form = '%%0%dx' % ml
    ...
    def decrypt(self, data):
        ...
        C3 = data[len_2:len_3]
        C2 = data[len_3:]
        xy = self.kg(int(self.private_key, 16), C1)
        x2 = xy[0:self.para_len]
...
def gen_key_pair():
    kit = SM2Suite(0, 0)
    sk = random.randint(1, int(default_ecc_table['n'], 16) - 2)
    pk = kit.kg(sk, default_ecc_table['g'])
    return str(hex(sk)), pk


if __name__ == '__main__':
    test_sk, test_pk = gen_key_pair()
